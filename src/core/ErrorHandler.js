...
const ViewNotFoundException_1 = require("./exceptions/ViewNotFoundException");
...
class ErrorHandler {
     ...
     */
    static async handle(error, req, isDev = false) {
        ...
        if (!isDev || req.headers.get('accept') === 'application/json') {
            ...
            if (error instanceof ValidationException_1.ValidationException && details) {
...
${dim}${error.stack?.split('\n').filter(l => !l.includes('node_modules')).slice(0, 5).join('\n')}${reset}
     ...
     */
    static async renderErrorPage(error, req, status, code, stack) {
        ...
        const isViewError = error instanceof ViewNotFoundException_1.ViewNotFoundException;
        const viewInfo = isViewError ? {
            viewPath: error.viewPath,
            searchedPaths: error.searchedPaths,
        } : null;
        // Special handling for ValidationException
        const isValidationError = error instanceof ValidationException_1.ValidationException;
        const validationErrors = isValidationError ? error.details : null;
...
${fileContext.snippet}
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div class="bg-[#0f1520] rounded-lg p-3 text-xs mono space-y-1.5 max-h-48 overflow-y-auto">
                ${Array.from(req.headers).slice(0, 8).map(([k, v]) => `
     ...
     */
    static async getFileContext(stack) {
        try {
            ...
            if (invalidExtensions.some(ext => file.endsWith(ext)))
                return null;
