...
export interface CanxResponse {
    ...
    hotwire: (content: string, options?: {
        target: string;
        action?: 'replace' | 'update' | 'prepend' | 'append' | 'remove' | 'after' | 'before';
    }) => Response;
...
export type MiddlewareHandler = (req: CanxRequest, res: CanxResponse, next: NextFunction) => Promise<Response | void> | Response | void;
...
export type RouteHandler = (req: CanxRequest, res: CanxResponse) => Promise<Response> | Response;
...
export type ValidationRule = 'required' | 'string' | 'number' | 'boolean' | 'email' | 'url' | 'uuid' | 'date' | 'array' | 'object' | `min:${number}` | `max:${number}` | `length:${number}` | `regex:${string}` | `in:${string}` | `notIn:${string}` | `same:${string}` | `different:${string}`;
...
export type ApplicationEvent = 'server:start' | 'server:stop' | 'request:start' | 'request:end' | 'request:error' | 'database:connect' | 'database:disconnect' | 'cache:hit' | 'cache:miss';
